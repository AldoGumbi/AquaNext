...
export const updateGrupo = async (req, res) => {
    ...
    try {
        ...
        // Manejo específico de errores de horarios con clases próximas
        if (error.code === 'HORARIOS_CON_CLASES_PROXIMAS') {
            return res.status(400).json({
                data: false,
                message: error.message,
                error: true,
                error_code: 'HORARIOS_CON_CLASES_PROXIMAS',
                horarios_afectados: error.horariosAfectados
            });
        }
...
export const deleteGrupo = async (req, res) => {
    try {
        ...
        // Manejo específico de errores de horarios con clases próximas
        if (error.code === 'GRUPO_CON_HORARIOS_ACTIVOS') {
            return res.status(400).json({
                data: false,
                message: error.message,
                error: true,
                error_code: 'GRUPO_CON_HORARIOS_ACTIVOS',
                horarios_afectados: error.horariosAfectados
            });
        }
