...
export const insertCoupons = async (req, res) => {
  try {
    ...
    if(error.code === 'ER_DUP_ENTRY') { // Unique violation error code
      return res.status(400).json({
        data: false,
        message: 'El código del cupón ya existe. Por favor, utiliza un código diferente.',
        error : error.message
      })
    }
    res.status(500).json({
...
export const updateCoupon = async (req, res) => {
  try{
    // (1) Obtener los parametros y body de la solicitud
    const { id } = req.params;
    const data = req.body;

    // (2) Validar que el ID del cupón sea un número válido
    if (!id || isNaN(Number(id))) {
      return res.status(400).json({
        data: false,
        message: 'El ID del cupón es obligatorio y debe ser un número válido.'
    ...
    if(!data || typeof data !== 'object' || Object.keys(data).length === 0) {
      return res.status(400).json({
        data: false,
        message: 'Los datos del cupón son obligatorios y deben ser un objeto válido.'
      });
    }


    // (3) llamar al modelo para actualizar el cupón
    const coupon = await couponsModel.UpdateCoupon(id, data);

    if(!coupon) {
      return res.status(404).json({
        data: false,
        message: 'No se encontró el cupón de descuento para actualizar.'
      });
    }
    res.status(200).json({
      data: {
        ...
        fecha_fin: data.expiration_day,

        usos_actuales: data.uses,
        usos_maximos: data.max_uses,

        tipo: data.discount_type,

        valor : data.discount_type == "porcentaje" ? data.discount_porcentaje : data.discount_amount,

      },
      message: 'Cupón de descuento actualizado exitosamente'
    });
    ...
    console.log(error);
    res.status(500).json({
      message: 'Error al actualizar el cupón de descuento',
      error : error.message
    });
  }
}
...
export const deleteCoupon = async (req, res) => {
  try {
    ...
    const deleted = await couponsModel.DeleteCoupon(Number(id));
    if (!deleted) {
      return res.status(404).json({
        data: false,
        message: 'No se encontró el cupón de descuento para eliminar.'
      });
    }
    res.status(200).json({
      data: id,
      message: 'Cupón de descuento eliminado exitosamente'
    });
  } catch (error) {
    res.status(500).json({
      message: 'Error al eliminar el cupón de descuento',
      error : error.message
    });
  }
}
