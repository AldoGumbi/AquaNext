...
const calcularCuposAjustados = (disponibilidad, year, mes) => {
    ...
    const esMesPasado = mesConsulta < mesActualInicio;
    const esMesFuturo = mesConsulta > mesActualInicio;
    const diaMes = now.day;
    
    // Determinar si debemos aplicar ajustes
    // Solo aplicamos ajustes si:
    // 1. Es mes futuro, O
    // 2. Es mes actual Y estamos dentro del periodo de gracia (días 1-8)
    const debeAplicarAjustes = esMesFuturo || (esMesActual && diaMes <= 8);
