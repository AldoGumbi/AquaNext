...
class authModel {
	...
	static async signInModel(user) {
		try {
			...
			if(email){
				query = "SELECT id, username, email, rol, activo FROM usuarios WHERE  email = ? AND password = ? LIMIT 1";
				params = [email, password];
			}else if(username) {
				query = "SELECT id, username, email, rol, activo FROM usuarios WHERE  username = ? AND password = ? LIMIT 1";
