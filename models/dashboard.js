  ...
  try {
    ...
    const [statsRows] = await db.query(`
      ...
      LEFT JOIN (
        ...
        WHERE td.referencia_tipo = 'INSCRIPCION' AND t.cancelada = 0
    ...
    const [tabla] = await db.query(`
      ...
      WHERE i.activa = 1
        AND CURDATE() BETWEEN i.fecha_inscripcion AND i.fecha_fin AND t.cancelada = 0
...
static async getIngresos() {
  try {
    ...
    const [kpiRows] = await db.query(`
      ...
      JOIN transacciones t
        ON t.id = td.transaccion_id
       AND COALESCE(t.cancelada, 0) = 0 AND t.cancelada = 0
      WHERE td.referencia_tipo = 'INSCRIPCION'
    `);
