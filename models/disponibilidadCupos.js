        ...
        try {
            ...
            const [rows] = await db.query(`
                SELECT 
                    ...
                    COUNT(DISTINCT CASE 
                        WHEN mg.mensualidad_id IS NOT NULL 
                            AND m.cancelada = 0 
                            AND a.deleted = 0 
                            AND a.id IS NOT NULL 
                        THEN mg.mensualidad_id 
                    END) as cupos_ocupados,
                    COALESCE(h.cupo_maximo, 16) - COUNT(DISTINCT CASE 
                        WHEN mg.mensualidad_id IS NOT NULL 
                            AND m.cancelada = 0 
                            AND a.deleted = 0 
                            AND a.id IS NOT NULL 
                    ...
                    CASE 
                        ...
                        ELSE ROUND((COUNT(DISTINCT CASE 
                            WHEN mg.mensualidad_id IS NOT NULL 
                                ...
                                AND a.id IS NOT NULL 
                            THEN mg.mensualidad_id 
                        END) / COALESCE(h.cupo_maximo, 16)) * 100, 2)
                    ...
                    CASE
                        ...
                        WHEN COUNT(DISTINCT CASE 
                            WHEN mg.mensualidad_id IS NOT NULL 
                                ...
                                AND a.id IS NOT NULL 
                            THEN mg.mensualidad_id 
                        END) >= COALESCE(h.cupo_maximo, 16) THEN 'Lleno'
                        ELSE 'Parcialmente ocupado'
                    END as estado_disponibilidad,
                    CONCAT(p.nombre, ' ', p.apellido) as profesor_asignado,
                    
                    -- Información de alumnos actuales (sin NULLs)
                    CASE 
                        WHEN COUNT(CASE 
                            WHEN mg.mensualidad_id IS NOT NULL 
                                AND m.cancelada = 0 
    ...
    static async getAlumnosPorHorarioYMes(horarioId, year, mes) {
        try {
            const [rows] = await db.query(`
                ...
                INNER JOIN mensualidades m ON mg.mensualidad_id = m.id AND m.cancelada = 0
